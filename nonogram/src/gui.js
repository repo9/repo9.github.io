...
const Gui = class
{
	...
	constructor( themePath )
	{
		...
		// set up theme
		this.themePath = themePath || './css';

		// load theme stylesheet
		this.themeStylesheetPath = this.themePath + '/gui.css';
		link.rel                 = 'stylesheet';
		link.type                = 'text/css';
		...
		this.themeTemplatesPath = './themes/default' + '/templates';
		this.templates          = [
	...
	resize(){
		this.boardSize  = this.boardSizes[3];
	...
	draw( puzzle )
	{
		...
		document.querySelector(".puzzle-setting").style.display = 'block';  //
		document.querySelector(".puzzle-restart-blind").style.display = 'none';  //완료후 재시작
		document.querySelector(".puzzle-blind").style.display = 'none';  //완료후 재시작
		var puzzle_deteil_label = document.querySelector(".puzzle-deteil-label");
		...
		if(this.puzzle.width == 5){
			//console.log("draw: "+puzzle.width);
			//this.boardSize  = this.boardSizes[2];
			puzzle_deteil_label.innerText ="Lv.Beginner";
		}else if(puzzle.width == 10){
			//console.log("draw: "+puzzle.width);
			//this.boardSize  = this.boardSizes[1];
			puzzle_deteil_label.innerText ="Lv.Medium";
		}else if(puzzle.width == 15){
			//console.log("draw: "+puzzle.width);
			//this.boardSize  = this.boardSizes[0];
			puzzle_deteil_label.innerText ="Lv.Expert";
		}else{
			//console.log("draw: "+puzzle.width);
			//this.boardSize  = this.boardSizes[0];
			puzzle_deteil_label.innerText ="Lv.Special";
		}
		...
		document.querySelector( '.restart-btn' ).addEventListener( 'click', () =>{
			...
			window.localStorage.setItem('asvglhmi-GameCellSave', JSON.stringify(this.puzzle.cells));
			window.localStorage.setItem('asvglhmi-GameColumnHintSave', JSON.stringify(this.puzzle.columnHints));
			window.localStorage.setItem('asvglhmi-GameGridSave', JSON.stringify(this.puzzle.grid));
			//window.localStorage.setItem('asvglhmi-GameCreatorSave', JSON.stringify(this.puzzle.creator));
			window.localStorage.setItem('asvglhmi-GameheightSave', this.puzzle.height);
			window.localStorage.setItem('asvglhmi-GameWidthSave', this.puzzle.width);
			window.localStorage.setItem('asvglhmi-GameRowHintSave', JSON.stringify(this.puzzle.rowHints));
			window.localStorage.setItem('asvglhmi-GameTotalCellSave', this.puzzle.totalCells);

			//document.querySelector( '.puzzle-setting-popup' ).style.display = 'none'; //닫기
		...
		document.querySelector( '.beginner-btn' ).addEventListener( 'click', () =>{
			...
			document.querySelector( '.puzzle-setting-popup' ).style.display = 'none'; //닫기
			document.querySelector('.puzzle-deteil-label').innerText= "Lv.Beginner";
		} );
		...
		document.querySelector( '.solved-btn' ).addEventListener( 'click', () =>{
			console.log("solved");
		...
		document.querySelector( '.menual-btn' ).addEventListener( 'click', () =>{
			console.log("menual");

			document.querySelector( '.puzzle-menual-popup' ).style.display = 'block'; //열기
		} );

		document.querySelector( '.close-menual-popup-button' ).addEventListener( 'click', () =>{
			console.log("menual close");

			document.querySelector( '.puzzle-menual-popup' ).style.display = 'none'; //열기
		} );
	...
	drawGenerateControls()
	{
		Promise.all( this.templatesLoaded ).then( () =>
		{
			...
			generate     = document.querySelector( '.restart-btn' );
			chooseSelect = document.querySelector( '[data-nonogram-choose-predefined]' );
			boardSize    = document.querySelector( '[data-nonogram-board-size]' );
			reset        = document.querySelector( '[data-nonogram-game-reset]' );
			solve        = document.querySelector( '[data-nonogram-game-solve]' );
			...
			generate.addEventListener( 'click', () =>
			{
				...
				const widthSelect    = document.querySelector( '[data-nonogram-generate-width]' ),
					  ...
					  creator        = new Creator(),
					  puzzle         = creator.createRandom( 2, 2, null )
				;
				this.selectedExample = null;
				this.draw( puzzle );
				this._resizeBoardForAvailableScreen();
	...
	drawSolution()
	{
		Promise.all( this.templatesLoaded ).then( () =>
		{
			...
			this.puzzle.cells.forEach( ( cell ) =>
			{
				...
				//window.localStorage.setItem('asvglhmi-GameCellSave', this.puzzle);
	...
	_showPuzzleSolved()
	{
		...
		document.querySelector(".puzzle-restart-blind").style.display = 'block';
		document.querySelector(".puzzle-setting").style.display = 'none';
		document.querySelector(".puzzle-blind").style.display = 'block';  //완료후 재시작 

		window.localStorage.setItem('asvglhmi-GameCellSave', null);
		window.localStorage.setItem('asvglhmi-GameColumnHintSave', null);
		window.localStorage.setItem('asvglhmi-GameGridSave', null);
		//window.localStorage.setItem('asvglhmi-GameCreatorSave', JSON.stringify(this.puzzle.creator));
		window.localStorage.setItem('asvglhmi-GameheightSave', null);
		window.localStorage.setItem('asvglhmi-GameWidthSave', null);
		window.localStorage.setItem('asvglhmi-GameRowHintSave', null);
		window.localStorage.setItem('asvglhmi-GameTotalCellSave', null);
	}
