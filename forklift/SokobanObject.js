...
var numTextSpan = null;
var moveTextSpan = null;
var gFrontDiv = null;
var gBackDiv = null;
...
function Sokoban() {
	...
	if(!stageRecent()){
		...
		initPuzzleSet = stageRecent();
	}

	if(!puzzleRecent()){
		initPuzzleNumber = 1;
	}else{
		initPuzzleNumber = puzzleRecent();
	}
...
function Sokoban_updateStatus() {
    while(numTextSpan.firstChild)
	numTextSpan.removeChild(numTextSpan.firstChild);
	
    numTextSpan.appendChild(
		document.createTextNode(this.puzzleSet + " - " + this.puzzleNumber)
    );

    while(moveTextSpan.firstChild)
	moveTextSpan.removeChild(moveTextSpan.firstChild);
	

    moveTextSpan.appendChild(
		document.createTextNode((this.moveHistory.length || 0) + " MOVES")
    );
...
function Sokoban_loadPuzzle(newPuzzleSet, newPuzzleNumber) {
    ...
    //data save
    if(newPuzzleSet>0 && newPuzzleNumber>0){
    	stageSave(newPuzzleSet);
    	puzzleSave(parseInt(newPuzzleNumber));
	}else{
		stageSave('1');
    	puzzleSave(1);
	}
...
function Sokoban_reset() {
	...
	// Top side + status area + separators + padding + game area + button bar + bottom side
		gWindowFrontH = 20 + 15 + 2*6 + 2*5 + 12*puzzleH + 19 + 18;
...
function Sokoban_move(dx, dy) {
	...
	with(this.playerPosition) {
	    ...
	    if(dx==1 && dy==0){
    		startState.div.className = "playerR";
    	}else if(dx==-1 && dy==0){
    		startState.div.className = "playerL";
    	}else if(dx==0 && dy==1){
    		startState.div.className = "playerB";
    	}else if(dx==0 && dy==-1){
    		startState.div.className = "playerT";
    	}
