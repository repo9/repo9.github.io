...
const gamePlay = (()=>{
  ...
  const coinSize = Math.floor(cWidth/5),
    cardWidth = Math.floor(cWidth/5),
  ...
  const pHandX = Math.floor(cWidth*0.25)-cardWidth/2-xDif,
    ...
    handY = Math.floor(cHeight*0.2)-cardHeight/2;
  const pHand = [], bHand = [];
  let pValue, bValue;
...
//Game Mechanics
  ...
  function setbetOutcomePlayer(){
    betOutcome = 'P'; 
    //drawInitialCards();
    ctx.clearRect(0,0,cWidth,cHeight*.7);
    ctx.drawImage(setUp.miscImgMap.get('bitcoin'),cWidth*.19-coinSize/2,cHeight*0.55-coinSize/2,coinSize,coinSize);
  }

  function setbetOutcomeTie(){
    betOutcome = 'T';
    //drawInitialCards();
    ctx.clearRect(0,0,cWidth,cHeight*.7);
    ctx.drawImage(setUp.miscImgMap.get('bitcoin'),cWidth*0.5-coinSize/2,cHeight*0.55-coinSize/2,coinSize,coinSize);
  }

  function setbetOutcomeBanker(){
    betOutcome = 'B';
    //drawInitialCards();
    ctx.clearRect(0,0,cWidth,cHeight*.7);
    ctx.drawImage(setUp.miscImgMap.get('bitcoin'),cWidth*0.81-coinSize/2,cHeight*0.55-coinSize/2,coinSize,coinSize);
  }
...
//Game logic
  ...
  function drawInitialCards(){
    ...
    if(bet<=0){
      cusAlert("There's no amount to bet on.","main-box");
      return;
    }

    if(bet>balance){
      cusAlert("There's not enough money.","main-box");
      return;
    }
    ...
    if(betOutcome=='P'|| betOutcome=='T' || betOutcome=='B'){
      
    }else{
      cusAlert("Choose! player or tie or banker.","Buttons");
      return;
    }
  ...
  function determineWinner(){
    ...
    if(betOutcome!=outcome){
      ...
      alertm = alertm + "get "+payout+"$";
    }

    cusAlert(alertm,"main-box");
