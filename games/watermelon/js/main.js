(() => {
  ...
  var audio = new Audio("assets/sound/1.wav");
  ...
  Events.on(render, "afterRender", () => {
    if (isGameOver) {
      ...
      writeText(score, "start", 25, 60, 50);

      var bestScore =  Number(localStorage.getItem("scoreDFNJGAQOKS"));

      if(bestScore < score){
        localStorage.setItem("scoreDFNJGAQOKS", score);
      }

      writeText("best score: "+ bestScore , "start", 25, 100, 20);
  ...
  function writeText(text, textAlign, x, y, size) {
    ...
    ctx.strokeStyle = "#b37b50";
    ctx.strokeText(text, x, y);

    ctx.fillStyle = "#fbff80";
